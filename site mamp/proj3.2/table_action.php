...
<?php if($_SESSION['user']=='admin') {?> <th> Options </th> <?php }?>
	...
	<tbody>
	<?php 
                            $select = 'SELECT code_action,ida,action.responsable,action.etape,action.datedecision,action.dateecheance,code_even,evenement.id,action FROM evenement,action where action.typeevenement=evenement.id and action.action='.$ida.' and action.processus='.$idp;
                            $result = mysql_query($select ) ;
